...
export const getMatches = async (
  ...
  try {
    ...
    const groupedMatches = notScouted.reduce((acc, match) => {
      ...
      acc[key].push(match);
      return acc;
    }, {} as Record<string, TeamMatchData[]>);
    ...
    let matchKeyAndNumber: OutputtedMatch[] = [];
    if (
    ...
    let finalMatches: OutputtedMatch[] = [];
    if (
    ...
    const finalFormatedMatches: FinalFormattedMatch[] = [];
    ...
    for (const element of finalMatches) {
      ...
      const currData: FinalFormattedMatch = {
        tournamentKey: params.data.tournamentKey,
        matchNumber: element.matchNumber,
        matchType: inversedMatchTypes[element.matchType],
...
async function addScoutedTeamNotOnSchedule(
  req: Request,
  team: string,
  match: FinalFormattedMatch,
  scouterShifts:
    | Record<(typeof scouterScheduleMap)[number], Scouter[]>[]
    | null = null,
  currIndex = -1
) {
  try {
    const key =
      match.tournamentKey +
      "_" +
      matchTypeShortNames[match.matchType] +
      match.matchNumber +
      "_" +
      inversedScouterScheduleMap[team];
...
async function addScoutedTeam(
  req: Request,
  scouterShifts: Record<(typeof scouterScheduleMap)[number], Scouter[]>[],
  currIndex: number,
  team: (typeof scouterScheduleMap)[number],
  match: FinalFormattedMatch &
    Record<
      (typeof scouterScheduleMap)[number],
      FinalFormattedMatch[(typeof scouterScheduleMap)[number]] & {
        scouters: (Scouter | { name: string | null; scouted: boolean })[];
      }
    >
) {
  try {
